...
export function switchUiColor() {
  // チェックボックスの取得
  const toggleSwitch = document.querySelector('#switch1');
  const content = document.querySelector('.switch-content');
  ...
  toggleSwitch.addEventListener('change', () => {
    if (toggleSwitch.checked === false) {
      ...
      content.textContent = '通常(ライト)モードです';
      content.innerHTML =
